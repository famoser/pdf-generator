...
class ConfigurationValidator extends \App\Service\Report\Document\Pdf\Configuration\ConfigurationValidator
{
    /**
     * @param array $config
     * @param string $key
    ...
    public function fontFamily(array $config, string $key)
    {
        ...
        if (!\in_array($fontFamily, $fontFamilyValues, true)) {
            throw new \Exception($key . ' config must be a valid font family (one of ' . implode(',', $fontFamilyValues) . ')');
        }
