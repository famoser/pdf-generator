...
class InvoiceLayout
{
    private readonly Document\Content\Text\TextStyle $headerStyle;

    private readonly Document\Content\Text\TextStyle $bodyStyle;
    ...
    public function __construct(private readonly Document $document, string $headerFontPath, string $bodyFontPath)
    {
        ...
        $this->headerStyle = new Document\Content\Text\TextStyle($headerFont, 8);

        $bodyFont = $this->document->getOrCreateEmbeddedFont($bodyFontPath);
        $this->bodyStyle = new Document\Content\Text\TextStyle($bodyFont, 6);
    }

    public function withHeaderStyle(Document\Content\Text\TextStyle $textStyle)
    {
    }
